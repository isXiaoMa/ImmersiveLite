...
function fillSelects() {
  ...
  from.innerHTML = IT_LANGUAGES.map(
    (l) =>
      `<option value="${l.code}" ${l.code === draft.from ? "selected" : ""}>${l.name}</option>`,
  ).join("");
  to.innerHTML = IT_LANGUAGES.filter((l) => l.code !== "auto")
    .map(
      (l) =>
        `<option value="${l.code}" ${l.code === draft.to ? "selected" : ""}>${l.name}</option>`,
    )
    .join("");
  const llmOpts = (draft.llmConfigs || [])
    .map(
      (c) =>
        `<option value="llm:${c.id}" ${"llm:" + c.id === draft.service ? "selected" : ""}>${c.name}</option>`,
    )
    .join("");
  svc.innerHTML =
    IT_SERVICES.map(
      (s) =>
        `<option value="${s.id}" ${s.id === draft.service ? "selected" : ""}>${s.name}</option>`,
    ).join("") + llmOpts;
}
...
function renderLlmList() {
  ...
  (draft.llmConfigs || []).forEach((cfg, idx) => {
    ...
    card.querySelector(".llm-name").addEventListener("input", (e) => {
      draft.llmConfigs[idx].name = e.target.value;
    });
    card.querySelector(".llm-baseurl").addEventListener("input", (e) => {
      draft.llmConfigs[idx].baseUrl = e.target.value.trim();
    });
    card.querySelector(".llm-apikey").addEventListener("input", (e) => {
      draft.llmConfigs[idx].apiKey = e.target.value.trim();
    });
    card.querySelector(".llm-model").addEventListener("input", (e) => {
      draft.llmConfigs[idx].model = e.target.value.trim();
    });
    card
      .querySelector(".llm-test")
      .addEventListener("click", () => runLlmTest(idx, card));
...
/**
 * 新增一条空白大模型配置（不预填 baseUrl/model，仅靠 placeholder 引导填写）
 */
function addLlmConfig() {
  if (!Array.isArray(draft.llmConfigs)) draft.llmConfigs = [];
  const id = "custom-" + Date.now().toString(36);
  draft.llmConfigs.push({ id, name: "", baseUrl: "", apiKey: "", model: "" });
  renderLlmList();
}
...
function fillForm() {
  ...
  document.getElementById("styleFontSize").value =
    draft.style.fontSize || "inherit";
}
...
async function saveDraft() {
  ...
  if (resp && resp.ok) {
    ...
    // 通知当前页面即时应用（悬浮球位置/划词开关等 UI 变化，与 popup 行为一致）
    try {
...
async function runLlmTest(idx, card) {
  ...
  try {
    const resp = await chrome.runtime.sendMessage({
      ...
      texts: [
        "Hello! This is a translation test for the immersive translate extension.",
      ],
      from: "auto",
      to: draft.to,
...
async function init() {
  ...
  document
    .getElementById("styleFollowColor")
    .addEventListener("change", syncColorDisabled);
    ...
    (entries) => {
      for (const en of entries) {
        ...
        const link = navLinks.find(
          (a) => a.getAttribute("href") === "#" + en.target.id,
        );
        if (link) {
