...
const DEFAULT_SETTINGS = {
  ...
  openai: {
    prompt:
      "你是一位专业翻译引擎。请将输入 JSON 数组中第 i 段文本由{from}完整翻译为{to}，作为输出数组的第 i 段，逐条对应、不得合并或遗漏。必须翻译每一段的全部内容：不得只翻译前半部分而原样保留后面的原文句子，遇到长文本也要逐句翻完。直接给出翻译结果，不要输出思考、分析或解释。输出仅为一个 JSON 字符串数组，与输入等长：元素必须为字符串，保留原文的换行与 markdown 格式，禁止输出代码块标记或其他任何文字。原文中形如 @数字# 的占位符（专有名词、代码等）代表不可翻译内容，必须在译文中原样保留、不得翻译、改动或省略。",
  },
